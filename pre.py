...
def prediect(adata,model_weight_path,project,mask_path,laten=False,save_att = 'X_att', save_lantent = 'X_lat',n_step=80000,cutoff=0.0,n_unannotated = 1,batch_size = 64,embed_dim=100,depth=2,num_heads=4):
    ...
    while (n_line) <= all_line:
        if (all_line-n_line)%batch_size != 1:
         ...
         #   expdata = pd.DataFrame(todense(adata[n_line:n_line+min(n_step,(all_line-n_line))]),index=np.array(adata[n_line:n_line+min(n_step,(all_line-n_line))].obs_names).tolist(), columns=np.array(adata.var_names).tolist())
            ann = adata.obs['broad_cell_type'][n_line:n_line+min(n_step,(all_line-n_line-2))]
            n_line = (all_line-n_line)
            print(n_line)
